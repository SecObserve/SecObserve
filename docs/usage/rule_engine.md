...
* **General rules**: The `Re-evaluate` button when showing a single general rule applies that rule to the existing observations of all products where `Apply general rules` is enabled. Because the amount of observations can be large, the re-evaluation runs as a background task and its result can be checked in the `Background tasks` administration. Re-evaluating a general rule is restricted to superusers, like all other changes to general rules.

The re-evaluation performs a full rule engine pass on the affected observations. That means:

* Observations that do not match the rule anymore lose its effects, with an entry in the observation log. Re-evaluating a rule that is disabled or waiting for approval therefore only removes its effects, which makes disabling a rule followed by a re-evaluation the way to undo a general rule.
