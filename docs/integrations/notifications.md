...
> **Deprecation notice**
>
> Microsoft has announced to discontinue support for incoming webhooks in Teams in favor of Powerautomate workflows based webhooks. For compatibiliy
> with existing installations the old webhooks are still the defautl configuration.
...
To use the new (v2) workflow based webhooks, just copy the webhook generated by the workflow integration into the input field. The expected payload for the v2 webhooks differs from the old format.

Enable the switch `V2 webhook` to use the correct template for generating notifications.
